...
def augment_train_data_two_side(data):
    ...
    data_top10 = data.iloc[0: (data.shape[0] // 10), :]
    data_low10 = data.iloc[-(data.shape[0] // 10):, :]
    return pd.concat([data_low10, data, data_top10], axis=0, ignore_index=True)

data = pd.read_csv("./original_data/d_train.csv", index_col=False, encoding='GBK')
data = data.sort_values(by=[u"血糖"], ascending=False)
...
new_data.to_csv(r'd_train_augment_{}.csv'.format(datetime.datetime.now().strftime('%Y%m%d_%H%M%S')), index=False, encoding='utf-8')